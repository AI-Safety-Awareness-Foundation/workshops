# -*- coding: utf-8 -*-
"""testing_solutions_smaller.ipynb
...
indices_that_fire_mainly_on_select_digit = find_values_for_sole_digit(models[13], digit_to_analyze)
for idx in indices_that_fire_mainly_on_select_digit[:3]:
  visualize_ith_key_value(models[13].to("cpu"), idx)
...
sanity_check_kv_outputs(models[13], train_dataset[0][0].cpu())

# This will list the key-value pairs that write the value vectors with the largest magnitude.
list_top_kv_pair_idxs(models[13], train_dataset[0][0].cpu(), 7000)

visualize_image(train_dataset[0][0].cpu().squeeze())

# Just an example, and you can try different keys as well!
visualize_ith_key_value_on_image(models[13], 46328, train_dataset[0][0].cpu().squeeze())
...
list_top_kv_pair_idxs(models[13], train_dataset[0][0].cpu(), 5500)

# However, what do you see in the interaction between pair 20254 and the image?
...
visualize_ith_key_value_on_image(models[13].cpu(), 905, train_dataset[2019][0].cpu().squeeze())
...
# Let's see how many of these are in our 65536 hidden units model

mostly_zero_indices = find_mostly_zero_values(models[13], threshold=0.05)
print(f"{mostly_zero_indices.shape}")

# make a "pruned" model with them all knocked out
model_with_zeros_pruned = knock_out_ith_key(models[13], mostly_zero_indices)

# Below is a comparison of the accuracy of the original and pruned models

print("Original Model Accuracy: ")
print(accuracy_by_digit(models[13].to(DEVICE), test_loader))
...
combined_dot_product_dist(models[13], test_loader, 0)

combined_dot_product_dist(model_with_zeros_pruned, test_loader, 0)

def analyze_kv_specificity(model, threshold=0.2):
    """
    Analyzes the key-value pairs in a model to count how many are "specific"
    versus "random" based on a given activation threshold.

    A key-value pair is considered "specific" if its corresponding value vector
    has a value greater than the threshold for at least one digit. Otherwise,
    it's considered "random" or not specialized.

    Args:
        model (SimpleNN): The model to analyze.
        threshold (float): The activation threshold to determine specificity.
    """
    # The value vectors are the columns of the fc2 weight matrix.
    # The shape of fc2.weight is (num_digits, num_hidden_units).
    value_vectors = model.fc2.weight

    # For each key-value pair (column), find the maximum value across all digits.
    # This gives us a tensor of the highest activation for each pair.
    max_value_per_pair, _ = torch.max(value_vectors, dim=0)

    # A pair is specific if its maximum activation is greater than the threshold.
    num_specific_pairs = torch.sum(max_value_per_pair > threshold).item()

    total_pairs = model.hidden_dim
    num_random_pairs = total_pairs - num_specific_pairs

    print(f"--- Key-Value Pair Specificity Analysis (Threshold = {threshold}) ---")
    print(f"Total Key-Value Pairs: {total_pairs}")
    print(f"Specific Pairs (at least one value > {threshold}): {num_specific_pairs}")
    print(f"Random Pairs (all values <= {threshold}): {num_random_pairs}")
    print("----------------------------------------------------")

    return num_specific_pairs, num_random_pairs

# Example of how to use the function with the largest model:
print("Analyzing the largest model (65536 hidden units):")
analyze_kv_specificity(models[13], threshold=0.2)

# Example with the smallest model:
print("\nAnalyzing the smallest model (8 hidden units):")
analyze_kv_specificity(model_with_zeros_pruned, threshold=0.2)